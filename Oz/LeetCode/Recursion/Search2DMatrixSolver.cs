...
public class Search2DMatrixSolver
{
        ...
        int totalRows, int totalColumns)
    {
        ...
        if (rowMin == rowMax && colMin == colMax && rowMin <= lastRowIndex && rowMin >= 0 &&
            colMin <= lastColumnIndex &&
            colMin >= 0)
        {
        ...
        if (target < matrix[pivotRow][pivotColumn])
        {
            return SearchInSubMatrix(matrix, target, rowMin, pivotRow, colMin, pivotColumn, totalRows, totalColumns)
                   || SearchInSubMatrix(matrix, target, rowMin, pivotRow, pivotColumn + 1, colMax, totalRows,
                       totalColumns)
                   || SearchInSubMatrix(matrix, target, pivotRow + 1, rowMax, colMin, pivotColumn, totalRows,
                       totalColumns);
        }
